...
Чтение трека из заголовка окна десктопного клиента (`scripts/read-yandex-desktop-title.ps1`) учитывает в том числе процессы **Y.Music.exe** (Store), **Яндекс Музыка.exe** (локализованное имя) и **YandexMusic.exe**. В заголовке плеера может не быть текста «Яндекс Музыка» — для десктопного процесса это нормально.
